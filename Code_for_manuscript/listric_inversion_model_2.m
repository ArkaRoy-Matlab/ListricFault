% ***************************************************************
% *** Matlab code for optimizing Mode1 2 (Reverse listric fault) having exponential density
...
filename='error_energy_without_noise_model2.txt'; %file name for saving error energy
%Incorporating Gaussian noise
%g_obs=g_obs+sqrt(2).*randn(size(g_obs))+0;  filename='error_energy_with_noise_model2.txt'; %file name for saving error energy
...
%% Problem Definition
%%%%    Optimization of Model 2 (reverse listric fault) using PSO     %%%%
c1=1.4; c2=1.7; 
            ...
            best_error=(squeeze(err_enrgy(:,id)));
                ...
                colormap hsv
                clim=[-1000 -400]; %colorbar axis limit
               %positioning the color bar
                ...
                ylim([0 5000])
                % drawing the legend
                hleg = legend([p1 p2],'location','northeast');
                % legend atributes
                ...
                ylabel('Depth (m)')
                 ...
                 %axis limit
                 xlim([x_obs(1) x_obs(end)])
                 legend('Synthetic data','Optimized data','location','southeast');
                 %axis labelling
                 xlabel('Horizontal distance (m)')
                 ylabel('Anomaly (mGal)')
                 box on

% ***************************************************************
% *** Matlab code for optimizing real Godaveri basin listric fault having exponential density
...
%importing synthetic gravity anomaly data for Godaveri basin listric fault
data=importdata('grav_data_godaveri.txt');
x_obs=data(:,1).*1000; %observation points in m
g_obs=data(:,2);       %gravity anomalies in mGal
%Incorporating Gaussian noise
%g_obs=g_obs+sqrt(2).*randn(size(g_obs))+0; %comment this line for without noise case
...
c1=1.4; c2=1.7; 
            ...
            best_error=(squeeze(err_enrgy(:,id)))';
                ...
                listric_data=importdata('fault_data_godaveri.txt');
                xm=((listric_data(:,1))')*1000;
                ym=((listric_data(:,2))')*1000;
                ...
                colormap hsv
                clim=[-500 -50]; %colorbar axis limit
               %positioning the color bar
                ...
                ylim([0 5000])
                % drawing the legend
                hleg = legend([p1 p2],'location','southwest');
                % legend atributes
                hleg.String = {'Chakravarthi et al. 2017','Optimized Model'};
                %axis labelling
                ...
                ylabel('Depth (m)')
                 ...
                 %axis limit
                 xlim([x_obs(1) x_obs(end)])
                 legend('Chakravarthi et al. 2017','Optimized data','location','northeast');
                 %axis labelling
                 xlabel('Horizontal distance (m)')
                 ylabel('Anomaly (mGal)')
                 box on
fprintf('\t For real Model\n')
